...
mh_clinic_plot <- grocery_plot +
  ...
  geom_point(data = mh_clinics_sf, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2], size = Num_Clinics), color = '#721f81', alpha = 0.7, guide = "legend", shape=1) +
...
unhoused_pop_plot <- mh_clinic_plot +
  geom_sf(data = unhoused_locations_sf, aes(size = Responses, alpha = Responses), color = "#150e38", shape=17, fill = "#150e38") +  scale_alpha_continuous(range = c(0.1, 1), name = "Number of Unhoused Individuals") +
  #labs(title = "Equity Zones, MH Clinics, Grocery Stores + Unhoused Population") +
  theme_minimal()+
  theme(axis.text = element_blank(), axis.title = element_blank())+
  guides(color = guide_legend(ncol = 2))
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("plot_selector", "Select variables to plot", 
                  choices = c("Equity Zones", "Equity Zones + Grocery Stores", "Equity Zones + Grocery Stores + Mental Health Clinics", "Equity Zones + Grocery Stores + Mental Health Clinics + Unhoused Population"),
...
server <- function(input, output) {
  ...
  selected_plot <- reactive({
    switch(input$plot_selector,
           ...
           "Equity Zones + Grocery Stores" = grocery_plot_2,
           "Equity Zones + Grocery Stores + Mental Health Clinics" = mh_clinic_plot,
           "Equity Zones + Grocery Stores + Mental Health Clinics + Unhoused Population" = unhoused_pop_plot) 
  })
...
# preload graphs and show them based on selected buttons?
# troubleshoot graph load time 