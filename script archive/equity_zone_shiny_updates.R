...
# Export all datasets used in plots so they can be called directly in to the Shiny app

file_path <- "C:/Users/higgi/Documents/GitHub/Housing-and-Migrants-Chicago/outside_data.csv"

# Export data frame to CSV
write.csv(plot_data, file = file_path, row.names = FALSE)

write.csv(unhoused_locations_sf, file = file_path, row.names = FALSE)

write.csv(equity_zone_sf, file = , row.names = FALSE)

# FIRST MAP: EQUITY ZONES
# Simple map that shows equity zone distribution 
equity_zones_plot <- ggplot(data = unique(plot_data)) +
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("plot_selector", "Select variables to plot", 
                  choices = c("Equity Zones", "Equity Zones + Grocery Stores + Unhoused Population", "Equity Zones + Mental Health Clinics + Unhoused Population"),
